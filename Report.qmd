...
for filename in os.listdir(dir):
  ...
  try:
    tasks[filename] = ARCTask.ARCTask(filepath)
  except:
    print('ERROR: Could not process file %s' % filepath)
  
tasks['1.json'].show(False)
```
...
We can adapt the Shannon Entropy to two dimensions by treating the unique pixel colors as a probability distribution.

```{python}
import plotnine as pl
# plt.matshow(tasks['1.json'].train[0][0])
# plt.show()
plt.figure()
plt.hist(tasks['1.json'].train[0][1].ravel())
plt.show()
```

The Kullback–Leibler (KL) divergence between the distribution
...
```{python}
#| label: appendix-full-arc-sets
#| layout-nrow: 10
for name, task in sorted(tasks.items(), key=lambda pair: int(pair[0].split('.')[0])):
  task.show()
```
